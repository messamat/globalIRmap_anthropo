...
plan_preprocess <- drake_plan(
  ...
  path_GRDCgaugep = file.path(path_resdir, "spatialoutputs.gdb\\grdcstations_cleanjoin_anthropo"),
  path_GRDCgaugedir =  file_in(!!file.path(rootdir, "data\\GRDCdat_day")),
  path_GSIMgaugep = file.path(path_resdir, "GSIM\\GSIM.gdb\\GSIMstations_cleanriverjoin_anthropo"),
...
plan_setupdata <- drake_plan(
  ...
  #-------------------- set-up tasks -------------------------------------
  gauges_div = target(
    gaugestats_format[(disan_m3_pyr >= discharge_interval[1]) &
...
plan_runmodels <- drake_plan(
  ...
  table_allbm = target(
    ...
    transform = map(
      ...
      in_bmid = list('classif1', 'classif2'),
      .names = c('tablebm_classif1', 'tablebm_classif2'))
  ),
